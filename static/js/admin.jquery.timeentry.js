...
//		}
	...
	_parseTime: function(inst) {
		...
		var allowEmpty = this._get(inst, 'allowEmpty');
		if (!(inst.input.val() == '' && allowEmpty)) {
			this._showTime(inst);
		}
	},
	...
	_extractTime: function(inst, value) {
		...
		if (currentTime.length >= 2) {
			...
			hour = ((isAM || isPM) && hour > 11) ? 0 : hour;
			hour = (show24Hours && hour > 23) ? 0 : hour;
			hour = (hour < 0) ? 0 : hour;
			var minute = parseInt(currentTime[1], 10);
			minute = (isNaN(minute) ? 0 : minute);
			minute = (minute > 59 || minute < 0) ? 0 : minute;
			var second = (currentTime.length >= 3 ?
				parseInt(currentTime[2], 10) : 0);
			second = (isNaN(second) || !this._get(inst, 'showSeconds') ? 0 : second);
			second = (this._get(inst, 'showSeconds') && second > 60) ? 0 : second;
	...
	_setValue: function(inst, value) {
		if (value != inst.input.val()) {
			if (value == '' && this._get(inst, 'allowEmpty') || value!='') {
