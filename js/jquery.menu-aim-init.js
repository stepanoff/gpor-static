$(document).ready(function() {
	...
	function activateSubmenu(row, event) {
		var $row = $(row);
		var e = event;
		var isBrandingPic = $(e.relatedTarget).closest('div').hasClass('branding-pic');
		var isNewSubmenu = false;
		if ($nowCurrent) {
			isNewSubmenu = $nowCurrent.find('.b-header-menu__item__link').html() != $row.find('.b-header-menu__item__link').html();
		}
		...
		$row.find('.b-header-menu__item__sub').show();
	}
	...
	$('.b-header-menu__item__sub').mouseout(function (e) {
		deactivateSubmenu($(this).closest('.b-header-menu__item'), e);
	});
