...
var initialized = false,
    initCallbacks = [],
    currentOptions = {},
    $popup, $title, $content, $frame, $defaultFrame;
...
function init()
{
    ...
    $defaultFrame = $popup.find('.js_popup_frame');
    $frame = $popup.find('.js_popup_frame');
    ...
    $popup.setDefaultView = function ()
    {
        $frame.children('div').first().show();
        $frame.children('div#js_popup_custom_view').hide();
    };
    $popup.setCustomView = function (content)
    {
        $frame.children('div').first().hide();
        $frame.children('div#js_popup_custom_view').html(content).show();
    };
    ...
    $popup
        .delegate('.' + options.classes.close, 'click', $popup.popuphide);
...
function show(options, $popup, $title, $content)
{
    ...
    {
        $popup.setCustomView(options.content);
    }
...
function $button(_options)
{
    ...
    if (options.action)
        ...
        {
            options.action.apply(this, [currentOptions, $popup, $title, $content]);
        });
